...
class IPGD_weighted(PersonalizedAlgorithm):
    """
    Interpolated Personalized Gradient Descent
    This algorithm return only the accuracy results of a selection of clients (nb_main_client)
    """
    ...
    def run(self):
        ...
        for lam in self.lams:
            honest_clients = [Client(config, i, self.dataloaders[i]) for i in range(nb_honest)]
            # ---- NEW: build a weight tensor ----
            honest_sizes = torch.tensor([c.num_samples for c in honest_clients],
                                        dtype=torch.float32)
            mean_size    = honest_sizes.mean()

            # pretend each of the f Byzantine slots owns `mean_size` samples
            byz_sizes = torch.full((self.config['f'],), mean_size, dtype=torch.float32)

            weights = torch.cat([honest_sizes, byz_sizes])
            weights /= weights.sum()            # normalise once, Σwᵢ = 1
            ...
            for j in range(self.nb_main_client): 
                ...
                for epoch in trange(config['T']): 
                    ...
                    # params already contains honest + byz tensors in that order
                    stacked = torch.stack(params)          # shape (n, d)
                    # weighted mean: (d × n) · (n) → (d)
                    fed_avr_param = (stacked.t() @ weights.to(stacked.device)).t()
...
algorithms_dict = {"FedAvg": FedAvg, "IPGD": IPGD, "IPGDW": IPGD_weighted , "E_IPGD": E_IPGD, "SplitParameters": SplitParameters}