...
def _get_per_client_value(value, idx):
    """Return value[idx] if value is a list/tuple/ndarray, else value itself."""
    return value[idx] if hasattr(value, '__iter__') else value
...
def homogenous_mnist_train_test(config, m, test_m = 2000, batch_size = 16, alpha = None, seed=42, save_folder=None):
    ...
    for i in trange(config['n'] - config['f']):
        m_i      = _get_per_client_value(m, i)
        test_m_i = _get_per_client_value(test_m, i)

        idx = np.random.choice(len(dataset), m_i + test_m_i, replace=False)

        client_dataset = torch.utils.data.Subset(dataset, idx)
        client_train_dataset, client_test_dataset = torch.utils.data.random_split(
            client_dataset, [m_i, test_m_i]
        )
        ...
        client_test_loader = torch.utils.data.DataLoader(
            client_test_dataset,  batch_size=128,    shuffle=False
        )
...
def dirichlet_mnist_train_test(config, m, test_m = 2000, batch_size = 64, alpha = 1, seed=42, save_folder=None):
    ...
    for i in trange( config['n']-config['f']):
        ...
        selection_p = [p[x]  for  x in dataset.targets] 
        selection_p = selection_p/np.sum(selection_p)
        idx = np.random.choice(len(dataset), m_i+test_m_i, replace=False,  p=selection_p)
        client_dataset = torch.utils.data.Subset(dataset, idx)
        client_train_dataset, client_test_dataset = torch.utils.data.random_split(client_dataset, [m_i/(m_i+test_m_i), test_m_i / (m_i+test_m_i)])
