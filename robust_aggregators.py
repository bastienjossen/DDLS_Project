...
def nnm_trmean(aggregator, vectors, trim_frac=None):
    """
    First do NNM (minimum-diameter subset of size n-f),
    then run trimmed-mean on those survivors.
    """
    # 1) pick the n-f “closest” vectors
    idxs = misc.compute_min_diameter_subset(vectors,
                                            aggregator.nb_workers,
                                            aggregator.nb_byz)
    survivors = [ vectors[i] for i in idxs ]

    # 2) run trimmed-mean on those
    #    we abuse `trim_frac` to mean fraction to trim each side,
    #    but our trmean signature takes only nb_byz, so we ignore trim_frac
    return trmean(aggregator, survivors)
...
robust_aggregators = {"average": average, "trmean": trmean, "median": median, "geometric_median_old": geometric_median_old, "geometric_median": geometric_median,
                      ...
                      "krum_old": krum_old, "nnm_old": nearest_neighbor_mixing_old, "nnm_trmean": nnm_trmean}
