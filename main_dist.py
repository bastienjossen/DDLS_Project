...
import os
import json
import src.dataloaders_dist
from src.algorithms import *
import logging
from losses_replot import plot_losses
from src.dataloaders_dist import *
import string
import random
...
parser.add_argument('--eval_every', type=int, default=1, metavar='EE')
parser.add_argument(
    '--m_list',
    type=str,
    default=None,
    metavar='MLIST',
    help='comma-separated list with one training-set size per honest client '
         '(overrides --m). Example: "32,64,128,128,256"'
)
args = parser.parse_args()

if args.m_list is not None:
    m_list = [int(x) for x in args.m_list.split(',')]
    if len(m_list) != args.n - args.f:
        raise ValueError(
            f'--m_list must contain exactly {args.n - args.f} integers '
            '(one per honest client)'
        )
else:
    m_list = [args.m] * (args.n - args.f)
...
dataset_folder = f'./experiments_distribution/{config["dataset"]}'
if not os.path.isdir(dataset_folder):
	os.mkdir(dataset_folder)
heterogeneity_folder = f'./experiments_distribution/{config["dataset"]}/{config["heterogeneity"]}'
if not os.path.isdir(heterogeneity_folder):
	os.mkdir(heterogeneity_folder)

dataset_folder = f'./experiments_distribution/{config["dataset"]}'
heterogeneity_folder = f'{dataset_folder}/{config["heterogeneity"]}'

os.makedirs(heterogeneity_folder, exist_ok=True)



# Settin up the folder where to store the plots and the experiments
save_folder = f'./experiments_distribution/{config["dataset"]}/{config["attack"]}/{config["heterogeneity"]}/n_{config["n"]}_m_{config["m"]}_f_{config["f"]}_T_{config["T"]}_runs_{config["nb_run"]}_alpha_{config["alpha"]}'
...
for run in range(config['nb_run']):
	start = time.time()
	print("Run ", run)
	dataloaders =  dataloading_func(config, m_list, config["test_m"], batch_size = config["batch_size"], alpha = config["alpha"], seed = seeds[run], save_folder = save_folder) 
